...
public class CapabilityMinecartDestination implements IGUITextFieldSensitive{
    ...
    public boolean useFuel(EntityMinecart cart){
        if(motorized) {
            ...
            if(fuelLeft > 0) {
                fuelLeft--;
                double randX = cart.getPositionVector().xCoord + (cart.world.rand.nextDouble() - 0.5) * 0.5;
                double randY = cart.getPositionVector().yCoord + (cart.world.rand.nextDouble() - 0.5) * 0.5;
                double randZ = cart.getPositionVector().zCoord + (cart.world.rand.nextDouble() - 0.5) * 0.5;
        ...
        if(!cart.world.isRemote) {
            if(isMotorized()) {
                ...
                if(shouldRun && useFuel(cart)) {
                    if(!motorActive) NetworkHandler.sendToAllAround(new PacketUpdateMinecartEngineState(cart, true), new NetworkRegistry.TargetPoint(cart.world.provider.getDimension(), cart.getPositionVector().xCoord, cart.getPositionVector().yCoord, cart.getPositionVector().zCoord, 64));
                    motorActive = true;
            ...
            if(motorActive) {
                cart.world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, cart.getPositionVector().xCoord, cart.getPositionVector().yCoord, cart.getPositionVector().zCoord, 0, 0, 0);
            }
        }
    }
